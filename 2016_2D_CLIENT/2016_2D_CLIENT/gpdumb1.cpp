...
if (!(bob->attr & BOB_ATTR_VISIBLE))
   return(1);
	...
	D3DXVECTOR3 pos = D3DXVECTOR3((bob->x - g_left_x) * 65 + 8, 
		(bob->y - g_top_y) * 65 + 8, 0.0);
	...
	if (bob->message_time > GetTickCount() - 2000) 
		Draw_Text_D3D(bob->message,pos.x,pos.y,D3DCOLOR_ARGB(255,200,200,255));
...
if (source_bitmap->attr & BITMAP_ATTR_LOADED)
   {
	D3DXVECTOR3 pos = D3DXVECTOR3(source_bitmap->x, source_bitmap->y, 0.0);
...
if (source_bitmap->attr & BITMAP_ATTR_LOADED)
   {
	D3DXVECTOR3 pos = D3DXVECTOR3(x, y, 0.0);
